...
int main() {
	...
	for (index = 0, index_window = 0; index_window < K; ++index, ++index_window) {
		if (index >= real_N)
			break;

		if (0 <= users[index].arrive_seconds)
			windows[index_window] = users[index].arrive_seconds + users[index].processing_minutes * 60;
		else {
			wait_seconds += 0 - users[index].arrive_seconds;
			windows[index_window] += users[index].processing_minutes * 60;
		}
	}
	int min_index_window, min_element_window;
	for (; index < real_N; ++index) {
